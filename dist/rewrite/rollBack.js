...
const getKeyByValue_1 = __importDefault(require("../utils/getKeyByValue"));
        ...
        try {
            ...
            if (ProjectRegExp.test(data)) {
                data.match(ProjectRegExp).forEach((url) => {
                    url = ProjectRule(url);
                    let newUrl = getKeyByValue_1.default(hashMap, url);
                    if (newUrl !== undefined) {
...
rollBack(listFile_1.default(ProjectPath));
